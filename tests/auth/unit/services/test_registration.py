...
def test_registration_create_user():
    ...
    registration(schema=schema, uow=uow)
...
def test_registration_user_with_this_username_exists():
    ...
    with pytest.raises(UserWithUsernameExists, match=error_text):
        registration(schema=schema, uow=uow)
...
def test_registration_invalid_verification_uuid(mocker):
    ...
    with pytest.raises(BadVerificationUUID, match=error_text):
        registration(schema=schema, uow=uow)
...
def test_registration_verification_not_found():
    ...
    with pytest.raises(VerificationNotFound, match=error_text):
        registration(schema=schema, uow=uow)

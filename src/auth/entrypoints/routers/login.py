...
async def login(request: Request, username: str = Form(...), password: SecretStr = Form(...)) -> Tokens:
    try:
        tokens = services.login(username=username, password=password.get_secret_value(), ip_address=request.state.ip)
